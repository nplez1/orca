...
- New floating tabs, agents, and terminals start in the floating workspace's own folder, `~/.orca/floating-workspace`, so they never inherit a repo or your home directory as a working directory. Pick a different folder under the same setting if you want one.
- Put floating-workspace agent instructions in that folder's `AGENTS.md` (Orca seeds a comment-only file there); agents launched from the floating panel read it from their working directory. Use **Show in Finder** / **Show in Explorer** next to the setting to open the folder.
- Floating markdown notes are saved in that same folder, even when you point the terminal directory somewhere else, so notes, agent instructions, and scratch files stay together.
