...
1. **`worktree.sharedDirectories` in `orca.yaml`** — repo-checked-in list of **gitignored directories** to materialize. By default these are symlinked so writes stay shared; Repository settings can instead try an APFS clone and choose a symlink, independent copy, or skip when cloning is unavailable. Clones and copies are not shared-write directories. The copy fallback has no size or entry-count cap and may be slow or use substantial disk space. SSH worktrees do not materialize shared directories yet. On POSIX, a narrow check-to-rename race can replace a concurrently created empty destination directory, detaching handles already opened to it; non-empty targets are preserved. If Orca crashes during materialization and the nested path is later removed from configuration or its ownership marker is missing, the staging directory may need manual cleanup. Use this for large rebuildable trees like `node_modules` or `.cache`. Entries must exist as directories in the primary checkout **and** be gitignored; tracked or missing paths are skipped.
