...
By default a click jumps straight to that agent's workspace and focuses its pane. Set **Clicking a card** to **Preview** (dashboard **board settings** gear, or **Settings → Experimental → Agent Dashboard**) to open the live terminal preview first — it has its own **Open worktree** button for the same jump. Both surfaces share the setting.
...
- Click a card to jump to that agent's workspace (or open its live terminal preview, per the board's **Clicking a card** setting)
