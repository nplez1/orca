...
- **Agents** — toggle individual CLIs on or off (Claude, Codex, Hermes, Pi, OMP, Prime Agent, Cursor, Gemini, Antigravity, Rovo Dev, Copilot, OpenCode, Grok, OpenClaw, Devin, Droid, Kimi). An agent you turned off in **Settings → Agents** is not offered here and its sessions are left out of the panel, so the menu only lists CLIs you actually use. Unchecked agents are skipped during the scan. Use **Select all** / **Clear** in the Agents header to flip every listed agent at once — **Clear** leaves none selected so you can turn on only the CLI you care about without unchecking a long list. An empty selection shows **No agents selected**, pointing at both the Agents menu and **Settings → Agents**.
