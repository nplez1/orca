...
Fireworks.ai is not a CLI agent, so there is nothing to launch. Orca reads its usage directly from an API key you paste in **Settings → Accounts**; see [Usage & rate-limit tracking](/docs/agents/usage-tracking).
