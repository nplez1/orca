...
- **Worktree Shared Paths** — gitignored paths to materialize from the primary checkout into each new worktree (APFS clone-copy on macOS when possible, otherwise symlink). Choose how repo-checked-in `worktree.sharedDirectories` from `orca.yaml` are handled: keep symlinks, try APFS clones with a symlink or copy fallback, or skip when cloning is unavailable. Clones and copies are independent per worktree; only symlinks share writes. The copy fallback has no size or entry-count cap and may be slow or use substantial disk space. SSH worktrees do not materialize shared directories yet. On POSIX, a narrow check-to-rename race can replace a concurrently created empty destination directory, detaching handles already opened to it; non-empty targets are preserved. If Orca crashes during materialization and the nested path is later removed from configuration or its ownership marker is missing, the staging directory may need manual cleanup. `.worktreeinclude` remains a separate, budget-limited copy feature (see [Worktrees](/docs/model/worktrees)).

## Floating Workspace

- **Enable Floating Workspace** — global surface for terminal, browser, and markdown tabs that are **not** tied to a repo worktree.
- **Terminal Directory** — start directory for new floating terminals and agents. Defaults to `~/.orca/floating-workspace`, the floating workspace's own folder; pick any folder to change it. Floating markdown notes always live in that folder, next to its `AGENTS.md` instructions.
...
- **Agent Dashboard** — kanban of Needs You / Working / Done agents (optional Idle), with search and project/workspace/PR filters; open in-window or as a pop-out. Clicking a card opens the agent's workspace by default, or its live terminal preview. **Show idle agents** lives on the dashboard board settings control, not here; **Open as** and **Clicking a card** live both here and on that control. See [Agents & sessions](/docs/model/agents-sessions#agent-dashboard).
