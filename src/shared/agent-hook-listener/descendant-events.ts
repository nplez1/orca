 ...
 *  rollout reconciliation; Muse's carries its own child-session filter. Each already
 *  implements the pane rule, so the generic path must not run a second, blinder copy
...
export function readDescendantEventFacts(
  ...
  switch (source) {
    // Why: these own their rosters; `providerOwnsDescendantLifecycle` already routes around this,
