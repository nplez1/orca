...
const ORCA_DOMAIN = 'com.nplez1.orca'
...
describe('ensureMacPressAndHoldDefault', () => {
  ...
  describe('domains we do not own', () => {
    ...
    it('accepts Orca and its channel-scoped bundles, and nothing else', () => {
      expect(isOrcaPreferencesDomain('com.nplez1.orca')).toBe(true)
      expect(isOrcaPreferencesDomain('com.nplez1.orca.dev')).toBe(true)
      ...
      expect(isOrcaPreferencesDomain('com.nplez1.orcafake')).toBe(false)
    })
  })
  ...
  it('reads CFBundleIdentifier from the plist beside the executable', () => {
    const exe = bundleWithPlist(
      '<plist><dict>\n<key>CFBundleName</key>\n<string>Orca</string>\n' +
        '<key>CFBundleIdentifier</key>\n\t<string>com.nplez1.orca</string>\n</dict></plist>'
    )

    expect(readBundleIdentifierFromExecutablePath(exe)).toBe('com.nplez1.orca')
  })
