...
export function getPiAgentStatusAsyncSubagentSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    '  // but bus listeners accumulate, so bind at most once. Deliberately a BLOCK and not',
    ...
    '    const processBus = process as unknown as { on?: (event: string, listener: (payload: unknown) => void) => void }',
    ...
    '      postAsyncSubagentState()',
    '    }',
    '    // Why: pi.events is the bus pi-subagents publishes async child runs on (a private',
    '    // EventEmitter, not `process`). The process channel stays bound for host builds that',
    '    // publish there; handler output is a full live set, so a duplicate is harmless.',
    "    bus?.on?.('subagent:async-started', onAsyncStarted)",
    "    bus?.on?.('subagent:async-complete', onAsyncComplete)",
    "    processBus.on?.('subagent:async-started', onAsyncStarted)",
    "    processBus.on?.('subagent:async-complete', onAsyncComplete)",
