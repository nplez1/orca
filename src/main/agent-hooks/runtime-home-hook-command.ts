...
export function wrapRuntimeHomeHookCommand(
  ...
  const windowsScript = `"\${HOME-}/${HOME_DIRECTORY_NAME}/agent-hooks/${scriptBaseName}.cmd"`
  const posixScript = `"\${HOME-}/${HOME_DIRECTORY_NAME}/agent-hooks/${scriptBaseName}.sh"`
  ...
  const powershellCommand = `$homePath = $env:HOME -replace '^/([A-Za-z])/', '$1:/'; $scriptPath = Join-Path $homePath '.${HOME_DIRECTORY_NAME}\\agent-hooks\\${scriptBaseName}.cmd'; if (Test-Path -LiteralPath $scriptPath -PathType Leaf) { & $scriptPath; exit $LASTEXITCODE }${powershellFallback}; ${WINDOWS_POWERSHELL_HOOK_ENVIRONMENT_GUARD}; [Console]::In.ReadToEnd() | Out-Null; exit 0`
