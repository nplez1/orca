...
> & {
  ...
  // Why: the DeepSeek credential is an API key stored outside GlobalSettings;
...
// Why: credits count as usage data — DeepSeek publishes no window at all, so
// without this term every window-shaped caller reads it as "no data" and hides
// a configured, funded account behind the invisible-bar state.
...
function createPendingProviderSnapshot(providerId: UsageProviderId): ProviderRateLimits {
  return {
    ...
    // Why: DeepSeek has no windows; its readout is the credits field.
    ...(providerId === 'deepseek' ? { credits: null } : {}),
    updatedAt: 0,
    error: null,
    status: 'fetching'
  }
