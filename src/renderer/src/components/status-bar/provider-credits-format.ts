...
/**
 * Money in the amount's own currency: a provider can answer in a currency other
 * than USD. `Intl` throws a RangeError on a currency code it does not recognize,
 * and a provider-supplied code crosses IPC unchecked, so an unformattable amount
 * must degrade rather than blank the whole status bar.
 */
